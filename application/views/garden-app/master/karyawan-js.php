	...
	function addmodal() {
      ...
      var html = '<table class="table table-condensed table-hover table-bordered" id="mytable">'+
		...
		var t = $("#mytable").dataTable({
		  ...
		  processing: true,
		  serverSide: true,
		  ajax: {"url": "<?= base_url('master/karyawan/json') ?>/", "type": "POST"},
		    columns: [
		  ...
		  columnDefs : [
		    { targets : [2],
		      render : function (data, type, row) {
		        return convertToRupiah(row['sallary']);
		      }
		    }
		  ],
     ...
     loaddata();
      ...
      $("#upload").submit(function(){
       ...
       function edit(id) {
     	...
     	$("#data-update").load('<?= base_url('master/karyawan/edit') ?>/'+id);
     }
     ...
     function hapus(id) {
     	...
     	var url = "<?= base_url('master/karyawan/delete') ?>/"+id;
